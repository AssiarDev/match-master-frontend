...
export const SelectorLeague = () => {
  ...
  const handleEnter = () => {
    ...
    const competitionId = (selectedTeam as unknown as { id_competition?: number })?.id_competition

    if (user) {
      addFavorite(user.id, selectedTeamId, competitionId as number)
    }
  ...
  return (
    <div className="w-full max-w-lg mx-auto px-4 flex flex-col gap-4 items-center mt-6">
      ...
      >
        {teams.map((team, i) => (
          <option key={i} value={team.id} className="text-white">
            {team.name}
          </option>
        ))}
      </select>
