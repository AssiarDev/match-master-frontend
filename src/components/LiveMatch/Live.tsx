...
export const Live = () => {
  ...
  const activeMatches = useMemo(
    () => matches.filter(m => m.state?.developer_name && INPLAY_STATES.has(m.state.developer_name)),
    [matches]
  )
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-4 sm:gap-6 py-6 sm:py-8 px-3 sm:px-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl sm:text-2xl font-bold text-white">Matchs en direct</h1>
        <div className="flex items-center gap-2">
          <span className={`h-2.5 w-2.5 rounded-full ${connected ? 'bg-green-500' : 'bg-gray-600'}`} />
          <span className="text-xs sm:text-sm text-gray-400">{connected ? 'Connecté' : 'Connexion...'}</span>
        </div>
      </div>
      ...
      {!connected && !error && (
        <p className="text-gray-400 text-center mt-10 text-sm">Connexion au flux en direct...</p>
      )}

      {connected && activeMatches.length === 0 && (
        <p className="text-gray-400 text-center mt-10 text-base sm:text-lg">Aucun match en direct pour le moment.</p>
      )}
