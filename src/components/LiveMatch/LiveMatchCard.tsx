...
const extractCurrentScore = (scores: LiveScore[] = []): { home: number; away: number } => {
...
const getGameTime = (periods: Period[] = []): string | null => {
    ...
    if (active.time_added > 0) return `${active.period_length}+${active.time_added}'`
    return `${active.minutes}'`
}

export const LiveMatchCard = ({ match }: { match: LiveMatch }) => {
    const home: LiveParticipant | undefined = match.participants?.find(p => p.meta.location === 'home')
    const away: LiveParticipant | undefined = match.participants?.find(p => p.meta.location === 'away')
    const { home: homeScore, away: awayScore } = extractCurrentScore(match.scores)
    ...
    return (
        <div className="border border-gray-700 rounded-xl shadow-md p-3 sm:p-4 w-full bg-zinc-900 text-white">
            ...
            <div className="flex items-center gap-2 mb-3">
                ...
                {gameTime && (
                    <span className="ml-auto text-xs font-bold text-red-400 tabular-nums">{gameTime}</span>
                )}
            </div>
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <span className="flex-1 text-sm font-medium truncate">{home?.short_code || home?.name}</span>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <span className="flex-1 text-sm font-medium truncate">{away?.short_code || away?.name}</span>
